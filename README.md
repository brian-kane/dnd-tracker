...
- **Skipped, not built:** a scheduled run also skips a card that already has an open or merged PR (checked against the live pull list, not GitHub's search index, which lags). A card that's failed `RETRY_CAP` (3) times is skipped too, with a one-time Trello comment flagging it for a human look instead of retrying it forever — a closed, unmerged PR and a run that produced no PR at all (the `flag-no-pr` comment below) each count as one strike toward that cap. A card carrying a comment whose exact text is `human-only` (its Done means needs something only a human can do — sending an email, testing on a phone, a retrospective judgment call) is skipped too, without counting as a strike; set it when approving a card into Up Next, or leave it for triage to recognize. If every card in Up Next is ineligible, the run exits cleanly and says how many were skipped as human-only. A manual dispatch with a card link ignores the marker and always tries to build that card.
...
- A workflow file change on its own branch can't test itself here: the Claude GitHub App's OIDC token exchange refuses to run unless the triggering branch's workflow file is byte-identical to `main`'s, so a PR touching `build.yml`/`review.yml` only really runs once merged. A `preflight` step (`preflight-workflow-matches-main.mjs`) enforces this: it's the first step of both jobs and fails immediately, before any Trello or agent work, when the running workflow file doesn't match the default branch's copy.
