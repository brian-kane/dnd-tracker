...
- **Workflow:** work is tracked in Trello. Lists: Parked, Up Next, Doing, Playtest, Done. Labels: Feature, Bug, Tooling. Card titles are "Subject: outcome". `/card` fetches the top card in Up Next (`/card <title>` for another), moves it to Doing, and to Playtest after merging.
...
- **`build.yml`/`review.yml` can't test themselves on their own branch:** the Claude GitHub App's OIDC token exchange refuses to authenticate unless the running workflow file is byte-identical to the default branch's. Each workflow's `preflight` step enforces this — it fails the job immediately, before any Trello or agent work, rather than just warning. A PR touching either file only really runs once merged; see README for the check.
